...
workbook = openpyxl.load_workbook('userinfos.xlsx')
...
def handle_message(update: Update, context):
    ...
    if useri.username:
        uname = useri.username
    else:
        uname = ' '
    if user_id not in subscriber_list:
        subscriber_list.append(user_id)
        worksheet.append([f'{user_id}', f'{useri.first_name}', f'{uname}'])
        print(f"New subscriber with ID {user_id}")
    else:
        print(f"Existing subscriber with ID {user_id}")
